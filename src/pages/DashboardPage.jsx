...
function LocationCard({
  ...
  // Calculate atmosphere by time of day
  const timeOfDayScores = useMemo(() => {
          ...
          <div className="grid grid-cols-4 gap-2 text-center mt-2">
            {Object.entries(timeOfDayScores).map(([period, pScore]) => (
              <div key={period} className="flex flex-col items-center">
                <span className="text-[9px] uppercase tracking-wider text-white/40">{period}</span>
                <div 
                  className="w-full h-1 mt-1 rounded-full" 
                  style={{ backgroundColor: pScore !== null ? atmosphereHealthFill(pScore) : '#333' }}
                />
              </div>
            ))}
          </div>
