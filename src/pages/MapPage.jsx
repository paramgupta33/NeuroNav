...
export default function MapPage() {
  ...
  const fetchLocations = useCallback(async () => {
    ...
    const [{ data: locData, error: locErr }, { data: repData }] = await Promise.all([
      ...
      supabase.from('sensory_reports').select('location_id, created_at, sound_level, crowd_level, lighting')
    ])

    if (locErr) {
      setLocations([])
    } else {
      const reportsByLoc = (repData ?? []).reduce((acc, r) => {
        if (!acc[r.location_id]) acc[r.location_id] = []
      ...
      const normalized = (locData ?? [])
        .map(loc => {
          ...
          return {
            ...norm,
            report_count: reportsByLoc[loc.id]?.length || 0,
            reports: reportsByLoc[loc.id] || []
          }
        })
        .filter(Boolean)
  ...
  const runPlaceSearch = useCallback(async () => {
    ...
    setSearchError(null)
    try {
      // Use Nominatim to get multiple results
      ...
      if (!results || results.length === 0) {
        setSearchError("🌾 We couldn't find that place. Try being more specific.")
        setSearchResults([])
        setShowDropdown(false)
        return
      }
      ...
      if (userCoords) {
        results.forEach(hit => {
          hit.distance = haversineKm(userCoords.lat, userCoords.lng, parseFloat(hit.lat), parseFloat(hit.lon))
        })
  ...
  }, [placeQuery, userCoords])

  const handleSelectSearchResult = async (hit) => {
    setShowDropdown(false)
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              {showDropdown && searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#1a1108] border border-white/10 rounded-xl shadow-2xl z-[2000] max-h-60 overflow-y-auto">
                  {searchResults.map((hit) => (
                    <button
                      key={hit.place_id}
                      onClick={() => handleSelectSearchResult(hit)}
                      className="w-full text-left px-4 py-3 hover:bg-white/5 border-b border-white/5 last:border-0 flex flex-col gap-1"
                    >
                      ...
                      {hit.distance !== undefined && (
                        <span className="text-[10px] text-grass font-bold uppercase tracking-widest">
                          {hit.distance.toFixed(1)} km away
                        </span>
                      )}
                    </button>
                  ))}
      ...
      <div className="h-full w-full">
        ...
        >
          ...
          {visibleLocations.map((loc) => {
            let timeOfDayScores = null;
            if (loc.reports && loc.reports.length > 0) {
              const periods = { Morning: [], Afternoon: [], Evening: [], Night: [] };
              loc.reports.forEach(r => {
                const hour = new Date(r.created_at).getHours();
                if (hour >= 6 && hour < 12) periods.Morning.push(r);
                else if (hour >= 12 && hour < 18) periods.Afternoon.push(r);
                else if (hour >= 18 && hour < 22) periods.Evening.push(r);
                else periods.Night.push(r);
              });
              timeOfDayScores = {
                Morning: periods.Morning.length ? atmosphereScore(periods.Morning) : null,
                Afternoon: periods.Afternoon.length ? atmosphereScore(periods.Afternoon) : null,
                Evening: periods.Evening.length ? atmosphereScore(periods.Evening) : null,
                Night: periods.Night.length ? atmosphereScore(periods.Night) : null,
              };
            }
                ...
                <div className="p-3 min-w-[180px]">
                  ...
                  {timeOfDayScores && (
                    <div className="grid grid-cols-4 gap-2 text-center mb-3">
                      {Object.entries(timeOfDayScores).map(([period, pScore]) => (
                        <div key={period} className="flex flex-col items-center">
                          <span className="text-[8px] uppercase tracking-wider text-soil/60">{period.substring(0, 3)}</span>
                          <div 
                            className="w-full h-1 mt-1 rounded-full" 
                            style={{ backgroundColor: pScore !== null ? atmosphereHealthFill(pScore) : '#e5e7eb' }}
                          />
                        </div>
                      ))}
                    </div>
